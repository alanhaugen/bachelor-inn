...
func minimax(state : GameState, depth : int) -> int:
	...
	if enemy_turn:
		...
		for move : Command in moves:
			best = max(best, minimax(state.apply_move(move), depth - 1))
		return best
	else:
		var best := INF
		for move : Command in moves:
			best = min(best, minimax(state.apply_move(move), depth - 1))
		return best
...
func choose_best_move(state : GameState, depth : int) -> Command:
	...
	for move in state.get_legal_moves():
		var score : int = minimax(state.apply_move(move), depth - 1)
